 ...
 */
register_uninstall_hook(SI_TIMESHEET_MODULE_NAME, 'si_timesheet_uninstall_hook');

function si_timesheet_uninstall_hook()
{
    $CI = &get_instance();
...
function si_timesheet_admin_init_hook()
{
	...
	if (is_admin() || has_permission('si_timesheet', '', 'view') || has_permission('si_timesheet', '', 'view_own')) {
		...
		$default_template_id_s = $CI->si_timesheet_model->get_default_template(get_staff_user_id(),SI_TS_FILTER_TYPE_SUMMARY);
		$default_template_id_c = $CI->si_timesheet_model->get_default_template(get_staff_user_id(),SI_TS_FILTER_TYPE_CALENDAR);
		...
		$CI->app_menu->add_sidebar_children_item('si_timesheet_menu', [
			...
			'href'     => admin_url('si_timesheet'.($default_template_id_c > 0 ? '?filter_id='.$default_template_id_c : "")),
			'position' => 1,
		]);
		$CI->app_menu->add_sidebar_children_item('si_timesheet_menu', [
			'slug'     => 'si-timesheets-summary',
			'name'     => _l('si_ts_timesheet_summary_menu'),
			'href'     => admin_url('si_timesheet/timesheet_summary'.($default_template_id_s > 0 ? '?filter_id='.$default_template_id_s : "")),
			'position' => 2,
		]);$CI->app_menu->add_sidebar_children_item('si_timesheet_menu', [
