...
class Si_timesheet_model extends App_Model
{
	...
	*/
	public function add($data)
	{
		if(isset($data['is_default']) && $data['is_default'] == 1)
			$this->remove_old_default_template($data['staff_id'], $data['filter_type']);
	...
	*/
	public function update($data,$filter_id)
	{
		...
		if(isset($data['is_default']) && $data['is_default'] == 1)
			$this->remove_old_default_template($data['staff_id'], $filter->filter_type);
	...
	* @param  mixed $staff_id staff id and filter_type
	* @return int
	*/
	public function get_default_template($staff_id, $filter_type = 1)
	{
		if($this->db->field_exists('is_default',db_prefix() . 'si_timesheet_filter')){
			$this->db->where('staff_id', $staff_id);
			$this->db->where('filter_type', $filter_type);
			$this->db->where('is_default', 1);
			$result = $this->db->get(db_prefix() . 'si_timesheet_filter')->row();
			if($result)
				return $result->id;
		}
		return "";
	...
	* @return boolean
	*/
	public function remove_old_default_template($staff_id, $filter_type = 1)
	{
		$this->db->where('staff_id',$staff_id);
		$this->db->where('filter_type', $filter_type);
		$update = $this->db->update(db_prefix() . 'si_timesheet_filter', ['is_default' => 0]);
		return $update;
	}
}
